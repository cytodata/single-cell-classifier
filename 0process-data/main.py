...
class DataGenerator():
    ...
    def getTrainCsv(self, csvFileName: str='training_data.csv') -> pd.core.frame.DataFrame:
        zipData = zipfile.ZipFile(self.locationZipFile, 'r')
        csvFile = pd.read_csv(zipData.open(csvFileName))
        return csvFile

    def getValidationCsv(self, csvFileName: str='validation_data.csv') -> pd.core.frame.DataFrame:
        zipData = zipfile.ZipFile(self.locationZipFile, 'r')
        csvFile = pd.read_csv(zipData.open(csvFileName))
        return csvFile