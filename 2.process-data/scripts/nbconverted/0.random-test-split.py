...
train_df, test_df = train_test_split(
    full_train_df, test_size=0.1, stratify=full_train_df.target, random_state=42
)
...
target_count_df = pd.DataFrame(train_df.target.value_counts()).merge(
    pd.DataFrame(test_df.target.value_counts()), left_index=True, right_index=True
)
